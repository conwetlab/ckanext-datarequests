...
ahoy cli "mkdir -p test/screenshots test/emails mail"
...
docker cp "$(docker-compose ps -q ckan)":/app/test/emails /tmp/artifacts/behave/
docker cp "$(docker-compose ps -q ckan)":/app/mail /tmp/artifacts/behave/emails/
